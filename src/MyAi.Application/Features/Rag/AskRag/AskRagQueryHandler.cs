...
public sealed class AskRagQueryHandler : IRequestHandler<AskRagQuery, AskRagResponse>
{
    ...
    public async Task<AskRagResponse> Handle(AskRagQuery request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();

        var queryEmbedding = await GenerateQueryEmbeddingAsync(request.Query, cancellationToken);
        var matches = await SearchAsync(queryEmbedding, request.TopK, cancellationToken);

        if (matches.Count == 0)
        {
            return new AskRagResponse(request.Query, NoRelevantContextAnswer, []);
        }
        ...
        return new AskRagResponse(request.Query, answer, sources);
    }
